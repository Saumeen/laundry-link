...
export default function StatusTransitionSelect({
  ...
  const { allowedTransitions, loadingTransitions, fetchTransitions } = useStatusTransitions();
  ...
  return (
    <div>
      ...
      {showLoadingState && loadingTransitions && (
        <p className='text-xs text-gray-500 mt-1'>Loading valid transitions...</p>
      )}
      {showLoadingState && !loadingTransitions && allowedTransitions.length === 0 && (
