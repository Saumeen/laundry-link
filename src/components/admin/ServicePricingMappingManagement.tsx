...
export default function ServicePricingMappingManagement() {
  ...
  const [editingMapping, setEditingMapping] =
    useState<ServicePricingMapping | null>(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/service-pricing-mappings/${id}`,
        {
          method: 'DELETE',
        }
      );
      ...
      <div className='flex justify-between items-center mb-6'>
        <div>
          ...
          <p className='text-gray-600 mt-1'>
            Manage relationships between services and pricing items
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className='mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg'>
          <div className='flex items-center space-x-2 text-sm'>
            ...
            <span className='text-blue-800 font-medium'>
              Editing: {getServiceName(editingMapping.serviceId)} →{' '}
              {getPricingItemName(editingMapping.pricingItemId)}
            </span>
          </div>
        </div>
      )}
          ...
          <div className='flex items-center justify-between mb-4'>
            <div>
              <h3 className='text-xl font-semibold text-gray-900'>
                {editingMapping
                  ? `Edit Mapping: ${getServiceName(editingMapping.serviceId)} → ${getPricingItemName(editingMapping.pricingItemId)}`
                  : 'Add New Mapping'}
              </h3>
              {editingMapping && (
                <div className='mt-1 text-sm text-gray-600'>
                  ...
                  <span className='mr-4'>
                    Status: {editingMapping.isActive ? 'Active' : 'Inactive'}
                  </span>
                  <span className='mr-4'>
                    Default: {editingMapping.isDefault ? 'Yes' : 'No'}
                  </span>
            ...
            <form onSubmit={handleSubmit} className='space-y-6'>
              <div className='grid grid-cols-2 gap-6'>
                ...
                <div>
                  ...
                  <p className='text-xs text-gray-500 mt-1'>
                    Select the pricing item to map
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className='text-xs text-gray-500 mt-1'>
                    Whether this mapping is available
                  </p>
                </div>
                ...
                <div className='col-span-2'>
                  ...
                  <p className='text-xs text-gray-500 mt-1'>
                    Mark as default mapping for this service
                  </p>
                </div>
              </div>
          ...
          <tbody>
            {mappings.map(mapping => (
              <tr key={mapping.id}>
                ...
                <td className='px-4 py-2 border'>
                  <span
                    className={`px-2 py-1 rounded text-xs ${mapping.isDefault ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                  >
                ...
                <td className='px-4 py-2 border'>
                  <span
                    className={`px-2 py-1 rounded text-xs ${mapping.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                  >
