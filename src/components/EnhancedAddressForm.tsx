...
interface EnhancedAddressFormProps {
  ...
  onAddressSelect: (suggestion: Record<string, unknown>) => void;
}
  ...
  const handleMapClick = useCallback(
    async (event: google.maps.MapMouseEvent) => {
      ...
      try {
        ...
        const geocodingResult = await googleMapsService.reverseGeocode(
          lat,
          lng
        );
        ...
        if (geocodingResult) {
          ...
          setFormData((prev: FormData) => ({
            ...
            locationType:
              (geocodingResult.locationType as
                | 'hotel'
                | 'home'
                | 'flat'
                | 'office') || 'flat',
    ...
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!window.google || !window.google.maps || !window.google.maps.places) {
        ...
        setErrors((prev: { [key: string]: string }) => ({
          ...prev,
          googleAddress:
            'Google Maps is not available. Please refresh the page and try again.',
        }));
        return;
      }
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-sm text-gray-600'>
                Click on the map, search for an address, or use your current
                location
              </p>
            </div>
            <button
        ...
        <div className='relative'>
          <div className='relative'>
            <input
              ...
              onFocus={() =>
                setShowSearchSuggestions(searchSuggestions.length > 0)
              }
      ...
      <div className='bg-white rounded-lg border border-gray-200 p-4'>
        ...
        {selectedAddress && selectedAddress.locationType && (
          <p className='mt-1 text-xs text-gray-500'>
            Location type automatically detected from the selected address. You
