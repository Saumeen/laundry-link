...
const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  const convertToUTC = useCallback((date: string, time: string): string => {
    if (!date || !time) return '';
    ...
    const bahrainDateTime = new Date(year, month - 1, day, hour, minute, 0, 0);
    
    // Get the timezone offset for Bahrain at this specific date/time
    const bahrainOffsetMs = getTimezoneOffsetForDate(bahrainDateTime, timezone);
  ...
  const getTimezoneOffsetForDate = useCallback((date: Date, timezone: string): number => {
  ...
  const convertFromUTC = useCallback((utcDateTime: string): { date: string; time: string } => {
    if (!utcDateTime) return { date: '', time: '' };
    
    const date = new Date(utcDateTime);
    const bahrainDate = date.toLocaleDateString('en-CA', { timeZone: timezone });
  ...
  const generateCalendarDays = useCallback(() => {
    ...
    const todayInTimezone = new Date(today.toLocaleDateString('en-CA', { timeZone: timezone }));
    ...
    for (let i = 0; i < 42; i++) {
      ...
      const isSelected = selectedDate ? date.toDateString() === new Date(selectedDate).toDateString() : false;
      const isDisabled = (!allowCurrentDay && date < todayInTimezone) || (minDate ? date < new Date(minDate) : false) || (maxDate ? date > new Date(maxDate) : false);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className={value ? 'text-gray-900' : 'text-gray-500'}>
                {value ? convertFromUTC(value).date + ' ' + convertFromUTC(value).time : placeholder}
              </span>
            </div>
