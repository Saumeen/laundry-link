...
import { EditCustomerModal, CustomerDetailsModal, ResetPasswordModal } from "@/components/admin/CustomerModals";
...
export default function CustomerManagement() {
  ...
  const fetchCustomers = useCallback(async (page: number, append = false) => {
    if (page === 1) {
  ...
  const handleToggleStatus = useCallback(async (customer: Customer) => {
    try {
      ...
      if (response.ok) {
        setMessage(`Customer ${customer.isActive ? 'deactivated' : 'activated'} successfully`);
  ...
  // Modal handlers
  const handleSaveCustomer = useCallback(async (customerId: number, updates: any) => {
    const response = await fetch('/api/admin/customers', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditAddress = useCallback(async (addressId: number, updates: any) => {
    ...
    if (selectedCustomer) {
      const customerResponse = await fetch(`/api/admin/customers?customerId=${selectedCustomer.id}`);
      if (customerResponse.ok) {
        const data = await customerResponse.json() as { customers: Customer[] };
        const updatedCustomer = data.customers.find(c => c.id === selectedCustomer.id);
        if (updatedCustomer) {
  ...
  const handleDeleteAddress = useCallback(async (addressId: number) => {
    const response = await fetch(`/api/admin/customer-addresses?id=${addressId}`, {
      method: 'DELETE'
    });
  ...
  const handleResetPassword = useCallback(async (customerId: number, newPassword?: string) => {
  ...
  useEffect(() => {
    ...
    if (session?.userType !== "admin" || (session?.role !== "SUPER_ADMIN" && session?.role !== "OPERATION_MANAGER")) {
      router.push("/admin/login");
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg font-medium">Loading Customer Management...</p>
          <p className="text-gray-500 text-sm mt-2">Please wait while we fetch customer data</p>
        </div>
      </div>
    );
  }
          ...
          {customers.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm || statusFilter ? 'Try adjusting your search criteria.' : 'No customers have been registered yet.'}
              </p>
            </div>
          )}
        </div>
