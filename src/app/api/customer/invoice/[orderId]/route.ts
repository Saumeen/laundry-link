...
export async function GET(
  ...
  try {
    ...
    doc.text(
      `Due Date: ${new Date(order.deliveryEndTime).toLocaleDateString()}`,
      20,
      yPosition + 17
    );
    doc.text(
      `Pickup Start: ${new Date(order.pickupStartTime).toLocaleString()}`,
      20,
      yPosition + 24
    );
    doc.text(
      `Pickup End: ${new Date(order.pickupEndTime).toLocaleString()}`,
      20,
      yPosition + 31
    );
    doc.text(
      `Delivery Start: ${new Date(order.deliveryStartTime).toLocaleString()}`,
      20,
      yPosition + 38
    );
    doc.text(
      `Delivery End: ${new Date(order.deliveryEndTime).toLocaleString()}`,
      20,
      yPosition + 45
    );
    doc.text(`Status: ${order.status}`, 20, yPosition + 52);
