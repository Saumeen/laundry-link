...
export async function POST(request: NextRequest) {
  try {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(`Date: ${new Date(order.createdAt).toLocaleDateString()}`, 120, 40);
    doc.text(`Due Date: ${new Date(order.deliveryTime).toLocaleDateString()}`, 120, 47);
    ...
    doc.text(`Delivery: ${new Date(order.deliveryTime).toLocaleString()}`, 20, 164);
    ...
    doc.text('For any questions, please contact us at info@laundrylink.bh', 20, 287);
