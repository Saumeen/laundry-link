...
export async function POST(request: Request) {
  try {
    ...
    // Get service names
    const serviceIds = (serviceUsageData as any[]).map(item => item.serviceId);
    ...
    // Get staff names
    const staffIds = (staffPerformanceData as any[]).map(item => item.staffId);
    const staffMembers = staffIds.length > 0 ? await prisma.staff.findMany({
      where: { id: { in: staffIds } },
      select: { id: true, firstName: true, lastName: true }
    }) : [];

    // Get driver names
    const driverIds = (driverPerformanceData as any[]).map(item => item.driverId);
    const drivers = driverIds.length > 0 ? await prisma.staff.findMany({
      where: { 
        id: { in: driverIds },
        role: { name: 'DRIVER' }
      },
    ...
    const totalRevenue = (summaryStats as any)._sum.invoiceTotal || 0;
    const totalOrders = (summaryStats as any)._count.id || 0;
    ...
    if (format === 'csv') {
      ...
      (orderStatusData as any[]).forEach((item: any) => {
        const percentage = totalOrders > 0 ? ((item._count.status / totalOrders) * 100).toFixed(1) : '0.0';
      ...
      (customerGrowthData as any[]).forEach((item: any) => {
      ...
      (serviceUsageData as any[]).forEach((item: any) => {
        const service = services.find(s => s.id === item.serviceId);
        const serviceName = service ? service.displayName : `Service ${item.serviceId}`;
      ...
      (staffPerformanceData as any[]).forEach((item: any) => {
        const staff = staffMembers.find(s => s.id === item.staffId);
        const staffName = staff ? `${staff.firstName} ${staff.lastName}` : `Staff ${item.staffId}`;
        const percentage = totalOrders > 0 ? ((item._count.staffId / totalOrders) * 100).toFixed(1) : '0.0';
      ...
      (driverPerformanceData as any[]).forEach((item: any) => {
        const driver = drivers.find(d => d.id === item.driverId);
        const driverName = driver ? `${driver.firstName} ${driver.lastName}` : `Driver ${item.driverId}`;
        const percentage = totalOrders > 0 ? ((item._count.driverId / totalOrders) * 100).toFixed(1) : '0.0';
    ...
    console.error('Error exporting report:', error?.message || error || 'Unknown error');
