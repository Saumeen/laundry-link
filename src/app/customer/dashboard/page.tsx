      ...
      >
        <div className='flex items-center justify-between mb-4'>
          <div className='flex items-center space-x-3'>
            <div
              className={`w-8 h-8 ${statusConfig.bgColor} rounded-full flex items-center justify-center`}
            >
    ...
    return (
      ...
      >
        <div className='flex items-center justify-between mb-4'>
          <div className='flex items-center space-x-3'>
            <div
              className={`w-10 h-10 ${statusConfig.bgColor} rounded-lg flex items-center justify-center`}
            >
        ...
        <div className='flex items-center justify-between'>
          <span
            className={`px-4 py-2 text-sm font-medium rounded-full border ${statusConfig.color}`}
          >
            {order.status}
          </span>
        </div>
...
function DashboardContent({ searchParams }: { searchParams: URLSearchParams }) {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/customer/orders?limit=${limit}&page=${page}&sort=updatedAt&order=desc`
      );
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    if (
      tab &&
      ['overview', 'orders', 'addresses', 'wallet', 'packages'].includes(tab)
    ) {
      setActiveTab(tab);
    }
  }, [searchParams]);
                  ...
                  {recentOrders.length === 0 ? (
                    <div className='text-center py-12'>
                      ...
                      <p className='text-gray-600 mb-6'>
                        Ready to get started? Place your first order!
                      </p>
                      <button
                        onClick={handleScheduleOrder}
            ...
            <div className='bg-white rounded-2xl shadow-sm border border-gray-100'>
              <div className='px-8 py-6 border-b border-gray-100'>
                ...
                <p className='text-gray-600 mt-2'>
                  Download and view your order invoices
                </p>
              </div>
              <div className='p-8'>
                {orders.length === 0 ? (
                  <div className='text-center py-12'>
                    ...
                    <p className='text-gray-600 mb-6'>
                      Invoices will be available once your orders are processed
                    </p>
                    <button
                      onClick={handleScheduleOrder}
                      className='bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl'
                      ...
                      >
                        <div className='flex items-center justify-between mb-4'>
                          ...
                          <div className='text-right'>
                            ...
                            <span
                              className={`px-3 py-1 text-xs font-medium rounded-full border ${getStatusConfig(order.status).color}`}
                            >
                              {order.status}
                            </span>
                          </div>
              ...
              <div className='p-8'>
                <div className='text-center py-12'>
                  ...
                  <p className='text-gray-600'>
                    Check back later for special offers and packages!
                  </p>
                </div>
              </div>
            </div>
